...
exports.getSession = async (req, res) => {
	...
	try {
		...
		console.log(
			`[Session] Fetching session data for user=${result.decoded.id}`,
		);
		...
		if (wallet) {
			...
			console.log(
				`[Session] Wallet fetched: wallet=${wallet._id}, balanceRaw=${wallet.balance}, balanceType=${typeof wallet.balance}, balanceNumber=${balanceNumber}, isValid=${!isNaN(balanceNumber)}`,
			);

			if (isNaN(balanceNumber) || !isFinite(balanceNumber)) {
				console.error(
					`[Session] Invalid balance value: wallet=${wallet._id}, balance=${wallet.balance} (type: ${typeof wallet.balance})`,
				);
				// Use 0 as fallback for invalid balance
			}
		} else {
			console.log(
				`[Session] No wallet found for user=${result.decoded.id}, returning default balance`,
			);
		}
		...
		console.log(
			`[Session] Session data ready: user=${user._id}, walletBalance=${userData.wallet.balance}`,
		);
